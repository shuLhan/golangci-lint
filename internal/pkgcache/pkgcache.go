...
func (c *Cache) Put(pkg *packages.Package, mode HashMode, key string, data interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to gob encode: " + err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf("failed to calculate package %s action id: %s", pkg.Name, err.Error())
	}
	c.ioSem <- struct{}{}
	...
	<-c.ioSem
	if err != nil {
		return fmt.Errorf("failed to save data to low-level cache by key %s for package %s: %s", key, pkg.Name, err.Error())
	}
...
func (c *Cache) Get(pkg *packages.Package, mode HashMode, key string, data interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to get data from low-level cache by key %s for package %s: %s", key, pkg.Name, err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf("failed to gob decode: " + err.Error())
	}
...
func (c *Cache) pkgActionID(pkg *packages.Package, mode HashMode) (cache.ActionID, error) {
	...
	if err != nil {
		return cache.ActionID{}, fmt.Errorf("failed to get package hash: " + err.Error())
	}
...
func (c *Cache) packageHash(pkg *packages.Package, mode HashMode) (string, error) {
	...
	for _, f := range pkg.CompiledGoFiles {
		...
		<-c.ioSem
		if fErr != nil {
			return "", fmt.Errorf("failed to calculate file %s hash: %s", f, fErr.Error())
		}
