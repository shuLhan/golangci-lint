...
func setupLintersFlagSet(v *viper.Viper, fs *pflag.FlagSet) {
	...
	internal.AddFlagAndBind(v, fs, fs.Bool, "disable-all", "linters.disable-all", false, "Disable all linters")

	internal.AddHackedStringSliceP(fs, "enable", "E", "Enable specific linter")
	internal.AddFlagAndBind(v, fs, fs.Bool, "enable-all", "linters.enable-all", false, "Enable all linters")

	internal.AddFlagAndBind(v, fs, fs.Bool, "fast", "linters.fast", false,
		"Enable only fast linters from enabled linters set (first run won't be fast)")

	internal.AddHackedStringSliceP(fs, "presets", "p",
		fmt.Sprintf("Enable presets (%s) of linters.\n"+
...
func setupRunFlagSet(v *viper.Viper, fs *pflag.FlagSet) {
	internal.AddFlagAndBindP(v, fs, fs.IntP, "concurrency", "j", "run.concurrency", getDefaultConcurrency(),
		"Number of CPUs to use (Default: number of logical CPUs)")
	...
	internal.AddFlagAndBind(v, fs, fs.String, "go", "run.go", "", "Targeted Go version")
	internal.AddHackedStringSlice(fs, "build-tags", "Build tags")

	internal.AddFlagAndBind(v, fs, fs.Duration, "timeout", "run.timeout", defaultTimeout, "Timeout for total work")
	...
	internal.AddDeprecatedFlagAndBind(v, fs, fs.Bool, "skip-dirs-use-default",
		"run.skip-dirs-use-default", true, getDefaultDirectoryExcludeHelp())
...
func setupOutputFlagSet(v *viper.Viper, fs *pflag.FlagSet) {
	internal.AddFlagAndBind(v, fs, fs.String, "out-format", "output.formats", config.OutFormatColoredLineNumber,
		fmt.Sprintf("Formats of output: %s", strings.Join(config.AllOutputFormats, "|")))
	...
	internal.AddFlagAndBind(v, fs, fs.StringSlice, "sort-order", "output.sort-order", nil,
		"Sort order of linter results")
	internal.AddFlagAndBind(v, fs, fs.String, "path-prefix", "output.path-prefix", "",
		"Path prefix to add to output")
	internal.AddFlagAndBind(v, fs, fs.Bool, "show-stats", "output.show-stats", false, "Show statistics per linter")
}

//nolint:gomnd // magic numbers here is ok
func setupIssuesFlagSet(v *viper.Viper, fs *pflag.FlagSet) {
	internal.AddHackedStringSliceP(fs, "exclude", "e", "Exclude issue by regexp")
	...
	internal.AddFlagAndBind(v, fs, fs.Bool, "exclude-case-sensitive", "issues.exclude-case-sensitive", false,
		"If set to true exclude and exclude rules regular expressions are case-sensitive")
	...
	internal.AddHackedStringSlice(fs, "exclude-files", "Regexps of files to exclude")
	internal.AddHackedStringSlice(fs, "exclude-dirs", "Regexps of directories to exclude")
...
func getDefaultIssueExcludeHelp() string {
	...
	for _, ep := range config.DefaultExcludePatterns {
		parts = append(parts,
			...
			fmt.Sprintf(`    Pattern: %s`, `'`+ep.Pattern+`'`),
		)
	}
